...
Mainnet:
 ...
 - [https://blockexplorer.exohood.com/](https://blockexplorer.exohood.com/) 🚧 Coming Soon
...
git clone --recursive -b release/4.0.0.0 http://github.com/exohood/exohood-blockchain
...
Once the exohood compiles, the explorer can be downloaded and compiled
as follows:

```bash
# go to home folder if still in ~/exohood
cd ~

# download the source code 
git clone https://github.com/exohood/exohood-blockchain.git

# enter the downloaded sourced code folder
cd exohood-blockchain
...
By default it will look for blockchain in its default location i.e., `~/.exohood/lmdb`.
You can use `-b` option if its in different location.

For example:

```bash
./exoblocks -b /home/mwo/non-defult-exohood-location/lmdb/
```

Example output:

```bash
[test@linux exohood-blockchain-explorer]$ ./exoblocks
2022-JUL-07 11:24:09.160280 Blockchain initialized. last block: 669785, d0.h0.m12.s47 time ago, current difficulty: 14786
(2022-07-07 09:03:49) [INFO    ] Crow/0.1 server is running, local port 8081
```
...
```
exoblocks, Onion exohood Blockchain Explorer:
  ...
  --enable-emission-monitor [=arg(=1)] (=0)
                                        enable exohood total emission monitoring
                                        thread
...
alias exoblocksmainnet='~/exohood-blockchain/build/exoblocks    --port 8081 --testnet-url "http://139.162.32.245:8082" --enable-pusher --enable-emission-monitor'

# for testnet explorer
alias exoblockstestnet='~/exohood-blockchain/build/exoblocks -t --port 8082 --mainnet-url "http://139.162.32.245:8081" --enable-pusher --enable-emission-monitor'
```

## Enable exohood emission

Obtaining current exohood emission amount is not straight forward. Thus, by default it is
disabled. To enable it use `--enable-emission-monitor` flag, e.g.,


```bash
exoblocks --enable-emission-monitor
```
...
Every 10000 blocks, the thread will save current emission in a file, by default,
 in `~/.exohood/lmdb/emission_amount.txt`. For testnet or stagenet networks,
 it is `~/.exohood/testnet/lmdb/emission_amount.txt` or `~/.exohood/stagenet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the
 explorer restarts, the thread will first check if `~/.exohood/lmdb/emission_amount.txt`
...
The values given, can be checked using exohood daemon's  `print_coinbase_tx_sum` command.
...
Having the `crt` and `key` files, run `exoblocks` in the following way:

```bash
./exoblocks --ssl-crt-file=/tmp/server.crt --ssl-key-file=/tmp/server.key
```
...
```bash
# we use here official exohood project's donation address as an example
...
We use recipient's address (i.e. not our address from which we sent exo to recipient).
...
```json
{
  "data": {
    ...
    "git_branch_name": "update_to_current_exohood",
    "last_git_commit_date": "2017-07-25",
    "last_git_commit_hash": "a549f25",
    "exohood_version_full": "0.10.3.1-ab594cfe"
  },
  "status": "success"
}
```
...
Return raw json block data, as represented in exohood.
...
Return raw json tx data, as represented in exohood.
...
## Other exohood examples

Other examples can be found on  [github](https://github.com/exohood).
