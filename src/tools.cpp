//
// Created by Ela Kuromi on 05/29/22.
//
...
/**
* Parse exohood address in a string form into
...
/**
* Return string representation of exohood address
*/
string
...
{
    // default path to exohood folder
    // on linux this is /home/<username>/.exohood
    string default_exohood_dir = tools::get_default_data_dir();

    if (nettype == cryptonote::network_type::TESTNET)
        default_exohood_dir += "/testnet";
    if (nettype == cryptonote::network_type::STAGENET)
        default_exohood_dir += "/stagenet";


    // the default folder of the lmdb blockchain database
    // is therefore as follows
    return default_exohood_dir + string("/lmdb");
}
...
{
    // the default folder of the lmdb blockchain database
    string default_lmdb_dir   = exohoodeg::get_default_lmdb_folder(nettype);
    ...
    blockchain_path = exohoodeg::remove_trailing_path_separator(blockchain_path);
...
{
    pair<uint64_t, uint64_t> sum_exo {0, 0};
    ...
    {
        sum_exo.first += vout["amount"].get<uint64_t>();
        ++sum_exo.second;
    }
...
{
    pair<uint64_t, uint64_t> sum_exo {0ULL, 0ULL};
        ...
        vector<txin_to_key_public>& input_public)
{
    ...
    uint64_t exo_outputs       {0};
    uint64_t exo_inputs        {0};
    uint64_t mixin_no          {0};
    ...
    for (size_t i = 0; i < input_no; ++i)
    {
        if(tx.vin[i].type() == typeid(cryptonote::txin_to_key))
        {
            ...
            exo_inputs += tx_in_to_key.amount;
    ...
    return {exo_outputs, exo_inputs, mixin_no, num_nonrct_inputs};
};
...
summary_of_in_out_rct(const json& _json)
{
    ...
    for (const json& vout: _json["vout"])
    {
        exo_outputs += vout["amount"].get<uint64_t>();
    }
    ...
    return {exo_outputs, exo_inputs, no_outputs, no_inputs, mixin_no, num_nonrct_inputs};
};
...
sum_money_in_inputs(const string& json_str)
{
    ...
    for (json& vin: j["vin"])
    {
        sum_exo.first += vin["key"]["amount"].get<uint64_t>();
        ++sum_exo.second;
    }
...
{
    array<uint64_t, 2> sum_exo;

    sum_exo[0] = sum_money_in_inputs(tx);
    sum_exo[1] = sum_money_in_outputs(tx);
