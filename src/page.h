//
// Created by JJ Uzumaki on 05/28/22.
//

#ifndef CROWEXO_PAGE_H
#define CROWEXO_PAGE_H



#include "mstch/mstch.hpp"

#include "exohood_headers.h"
...
/*
    ...
    // public keys and EXO amount of outputs
    ...
    get_mstch_map(crypto::public_key combined_key = crypto::null_pkey) const
    {
        ...
        const double& EXO_amount = EXO_AMOUNT(fee);

        // tx size in kB
        double tx_size =  static_cast<double>(size)/1024.0;

        double payed_for_kB = EXO_amount / tx_size;
        ...
        if (!input_key_imgs.empty())
        {
            ...
            fee_str          = fmt::format("{:0.2f}", EXO_amount);
            fee_short_str    = fmt::format("{:0.2f}", EXO_amount);
            fee_micro_str    = fmt::format("{:02.0f}" , EXO_amount * 1e2);
        ...
        mstch::map txd_map {
                ...
                {"tx_fee"            , fmt::format("{:0.2f}", EXO_amount)},
                ...
                {"sum_inputs"        , EXO_amount_to_str(EXO_inputs , "{:0.2f}")},
                {"sum_outputs"       , EXO_amount_to_str(EXO_outputs, "{:0.2f}")},
                {"sum_inputs_short"  , EXO_amount_to_str(EXO_inputs , "{:0.2f}")},
                {"sum_outputs_short" , EXO_amount_to_str(EXO_outputs, "{:0.2f}")},
                ...
                {"relative_balance"     , balance > 0 ? EXO_amount_to_str(balance , "{:0.2f}") : EXO_amount_to_str(balance * -1 , "{:0.2f}")},
                {"positive_relative_balance"     , balance > 0},
                {"negative_relative_balance"     , balance < 0},
                {"tx_timestamp"                        , exohoodeg::timestamp_to_str_gm(tx_timestamp) },
    ...
    get_decomposed_tx_outputs(set<std::string> &input_addresses, int64_t &balance, crypto::public_key combined_key) const
    {
      ...
      if(version >= 2) {
        ...
        for(auto i = 0; i < output_public.size(); ++i) {
          ...
          public_key cKey = exohoodeg::addKeys(txout_key.address.m_view_public_key, txout_key.address.m_spend_public_key);
          ...
          tx_outputs.push_back(mstch::map {
                  {"amount"         , exohoodeg::EXO_amount_to_str(output_public.at(i).second)},
                  {"to"             , to},
                  {"is_change"      , is_change},
                  {"is_current_EXO_address", combined_key == cKey},
                  {"is_stealth",    false}
          });
        }
      }
      else
      {
        ...
        for(auto i = 0; i < MAX_DISPLAY_IN_OUT && i < output_pub_keys.size(); ++i) {
          ...
          tx_outputs.push_back(mstch::map {
                  {"amount"         , exohoodeg::EXO_amount_to_str(output_pub_keys.at(i).second)},
      ...
      if(version >= 3) {
        if(input_public.empty())
        {
          ...
          for (auto i = 0; i < input_public.size(); i++)
          {
            ...
            std::string amount = exohoodeg::EXO_amount_to_str(in_public.amount);
      ...
      else
      {
        for(auto i = 0; i < MAX_DISPLAY_IN_OUT && i < input_key_imgs.size(); i++) {
          tx_inputs.push_back(mstch::map {
                  ...
                  {"amount"         , exohoodeg::EXO_amount_to_str(input_key_imgs.at(i).amount)},
                  {"is_stealth",    true}
          });
        }
      }
          ...
          addr_outs_cache(8000)
{
    ...
    template_file["css_styles"]      = exohoodeg::read(TMPL_CSS_STYLES);
    template_file["header"]          = exohoodeg::read(TMPL_HEADER);
    template_file["footer"]          = get_footer();
    template_file["index2"]          = get_full_page(exohoodeg::read(TMPL_INDEX2));
    template_file["mempool"]         = exohoodeg::read(TMPL_MEMPOOL);
    template_file["altblocks"]       = get_full_page(exohoodeg::read(TMPL_ALTBLOCKS));
    template_file["mempool_error"]   = exohoodeg::read(TMPL_MEMPOOL_ERROR);
    template_file["mempool_full"]    = get_full_page(template_file["mempool"]);
    template_file["block"]           = get_full_page(exohoodeg::read(TMPL_BLOCK));
    template_file["tx"]              = get_full_page(exohoodeg::read(TMPL_TX));
    template_file["my_outputs"]      = get_full_page(exohoodeg::read(TMPL_MY_OUTPUTS));
    template_file["rawtx"]           = get_full_page(exohoodeg::read(TMPL_MY_RAWTX));
    template_file["checkrawtx"]      = get_full_page(exohoodeg::read(TMPL_MY_CHECKRAWTX));
    template_file["pushrawtx"]       = get_full_page(exohoodeg::read(TMPL_MY_PUSHRAWTX));
    template_file["rawkeyimgs"]      = get_full_page(exohoodeg::read(TMPL_MY_RAWKEYIMGS));
    template_file["rawoutputkeys"]   = get_full_page(exohoodeg::read(TMPL_MY_RAWOUTPUTKEYS));
    template_file["checkrawkeyimgs"] = get_full_page(exohoodeg::read(TMPL_MY_CHECKRAWKEYIMGS));
    template_file["checkoutputkeys"] = get_full_page(exohoodeg::read(TMPL_MY_CHECKRAWOUTPUTKEYS));
    template_file["address"]         = get_full_page(exohoodeg::read(TMPL_ADDRESS));
    template_file["search_results"]  = get_full_page(exohoodeg::read(TMPL_SEARCH_RESULTS));
    template_file["tx_details"]      = exohoodeg::read(string(TMPL_PARIALS_DIR) + "/tx_details.html");
    template_file["tx_details_v2"]      = exohoodeg::read(string(TMPL_PARIALS_DIR) + "/tx_details_v2.html");
    template_file["tx_details_v3"]      = exohoodeg::read(string(TMPL_PARIALS_DIR) + "/tx_details_v3.html");
    template_file["tx_table_header"] = exohoodeg::read(string(TMPL_PARIALS_DIR) + "/tx_table_header.html");
    template_file["tx_table_row"]    = exohoodeg::read(string(TMPL_PARIALS_DIR) + "/tx_table_row.html");

    if (enable_js) {
        // JavaScript files
        template_file["jquery.min.js"]   = exohoodeg::read(JS_JQUERY);
        template_file["crc32.js"]        = exohoodeg::read(JS_CRC32);
        template_file["crypto.js"]       = exohoodeg::read(JS_CRYPTO);
        template_file["cn_util.js"]      = exohoodeg::read(JS_CNUTIL);
        template_file["base58.js"]       = exohoodeg::read(JS_BASE58);
        template_file["nacl-fast-cn.js"] = exohoodeg::read(JS_NACLFAST);
        template_file["sha3.js"]         = exohoodeg::read(JS_SHA3);
        template_file["config.js"]       = exohoodeg::read(JS_CONFIG);
        template_file["biginteger.js"]   = exohoodeg::read(JS_BIGINT);
...
index2(uint64_t page_no = 1, bool refresh_page = false)
{
    ...
    std::future<json> network_info_ftr = std::async(std::launch::async, [&]
    {
        ...
        get_exohood_network_info(j_info);
    ...
    if(height < 1175315)
    {
        alert_message = "<b>Blockchain Update:</b> We have a planned blockchain update set to occour on block 1175315 (approx. 30th September)."
            ...
            "or vendor that utilizes any of the blockchain or wallet software, please update your binaries to the latest exohood 4.0 Aurora release "
            "available at our <a href=\"https://github.com/exohood/exohood/releases\" style=\"\">GitHub page</a>."
            "<br><br>"
            "If you are an exohood App (Android/iOS) or Online Wallet user, no actions required.";
    ...
    {
        alert_message = "<b>Blockchain Update:</b> The blockchain was updated successfully to the new public version of exohood's blockchain. "
        "Transactions might take longer than usual to confirm due to the influx of migration transactions."
        "<br><br>"
        "If you are a Wallet-CLI user, exchange, "
        "or vendor that utilizes any of the blockchain or wallet software, please update your binaries to the latest exohood 4.0 release "
        "available at our <a href=\"https://github.com/exohood/exohood/releases\" style=\"\">GitHub page</a>."
        "<br><br>"
        "If you are an exohood App (Android/iOS) or Online Wallet user, no actions required.";
    ...
    mstch::map context {
            ...
            {"height"                   , height},
            {"server_timestamp"         , exohoodeg::timestamp_to_str_gm(local_copy_server_timestamp)},
    ...
    double blk_size_median = exohoodeg::calc_median(blk_sizes.begin(), blk_sizes.end());
    ...
    context["network_info"] = mstch::map {
            ...
            {"hash_rate"         , hash_rate},
            {"fee_per_kb"        , print_EXO(current_network_info.fee_per_kb)},
    ...
    if (CurrentBlockchainStatus::is_thread_running())
    {
        ...
        string emission_coinbase = EXO_amount_to_str_formated(current_values.coinbase, "{:0.2f}");
...
mempool(bool add_header_and_footer = false, uint64_t no_of_mempool_tx = 10)
{
    ...
    for (size_t i = 0; i < no_of_mempool_tx; ++i)
    {
        ...
        txs.push_back(mstch::map {
                ...
                {"EXO_inputs"      , mempool_tx.EXO_inputs_str},
                {"EXO_outputs"     , mempool_tx.EXO_outputs_str},
...
show_block(uint64_t _blk_height, size_t page_no = 1)
{
    ...
    string blk_timestamp = exohoodeg::timestamp_to_str_gm(blk.timestamp);
    ...
    context["sum_fees"]
            = exohoodeg::EXO_amount_to_str(sum_fees, "{:0.2f}", false);

    // get EXO in the block reward
    context["blk_reward"]
            = exohoodeg::EXO_amount_to_str(txd_coinbase.EXO_outputs - sum_fees, "{:0.2f}");
...
show_block(string _blk_hash, size_t page_no = 1)
{
    ...
    if (!exohoodeg::parse_str_secret_key(_blk_hash, blk_hash))
    {
...
show_tx(string tx_hash_str, uint16_t with_ring_signatures = 0, bool refresh_page = false)
{
    ...
    if (!exohoodeg::parse_str_secret_key(tx_hash_str, tx_hash))
    {
    ...
    if (!mcore->get_tx(tx_hash, tx))
    {
        ...
        if (!found_txs.empty())
        {
            ...
            blk_timestamp = exohoodeg::timestamp_to_str_gm(tx_recieve_timestamp);
...
show_ringmembers_hex(string const& tx_hash_str)
{
    ...
    vector<txin_to_key> input_key_imgs = exohoodeg::get_key_images(tx);
...
show_ringmemberstx_jsonhex(string const& tx_hash_str)
{
    ...
    // private exohood networks, so we can easly put these
                ...
                bool tx_prove = false)
{
    ...
    if (EXO_address_str.empty())
    {
        return string("exohood address not provided!");
    }
    ...
    // parse string representing given exohood address
    cryptonote::address_parse_info address_info;

    if (!exohoodeg::parse_str_address(EXO_address_str,  address_info, nettype))
    {
        cerr << "Cant parse string address: " << EXO_address_str << endl;
        return string("Cant parse EXO address: " + EXO_address_str);
    }
    ...
    if (!exohoodeg::parse_str_secret_key(viewkey_str, multiple_tx_secret_keys))
    {
...
//        if (!exohoodeg::parse_str_secret_key(spend_key_str, prv_spend_key))
//        {
...
//        }
    ...
    if (tx_blk_found)
    {
        ...
        blk_timestamp = exohoodeg::timestamp_to_str_gm(blk.timestamp);
    ...
    mstch::map context {
            ...
            {"EXO_address"          , EXO_address_str},
            {"viewkey"              , viewkey_str_partial},
            {"tx_pub_key"           , pod_to_hex(txd.pk)},
            {"blk_height"           , tx_blk_height_str},
            {"tx_size"              , fmt::format("{:0.4f}",
                                                  static_cast<double>(txd.size) / 1024.0)},
            {"tx_fee"               , exohoodeg::EXO_amount_to_str(txd.fee, "{:0.2f}", true)},
            {"blk_timestamp"        , blk_timestamp},
    ...
    string server_time_str = exohoodeg::timestamp_to_str_gm(server_timestamp, "%F");
    ...
    for (pair<txout_to_key, uint64_t>& outp: txd.output_pub_keys)
    {
        ...
        // if someone had sent us some EXO.
...
//             << pod_to_hex(tx_pubkey) << '\n'  << '\n';
        ...
        outputs.push_back(mstch::map {
                ...
                {"amount"                , exohoodeg::EXO_amount_to_str(outp.second)},
    ...
    // to hold sum of EXO in matched mixins, those that
    // perfectly match mixin public key with outputs in mixn_tx.
    uint64_t sum_mixin_EXO {0};
    ...
    // we get number of all posibilities, and their total EXO amount
    ...
    for (const txin_to_key& in_key: input_key_imgs)
    {
        ...
        inputs.push_back(mstch::map{
                ...
                {"key_image_amount", exohoodeg::EXO_amount_to_str(in_key.amount)},
        ...
        for (const uint64_t& abs_offset: absolute_offsets)
        {
            ...
            public_key mixin_tx_pub_key
                    = exohoodeg::get_tx_pub_key_from_received_outs(mixin_tx);
            ...
            output_pub_keys = exohoodeg::get_ouputs_tuple(mixin_tx);
...
//                        }
                ...
                found_outputs.push_back(mstch::map {
                        ...
                        {"amount"          , exohoodeg::EXO_amount_to_str(amount)}
                });
                ...
                if (mine_output)
                {
                    ...
                    // increase sum_mixin_EXO only when
...
//
//                        if (!exohoodeg::generate_key_image(derivation,
...
//                                << endl;
    ...
    context["found_our_outputs"] = (sum_EXO > 0);
    context["sum_EXO"]           = exohoodeg::EXO_amount_to_str(sum_EXO);

    context.emplace("inputs", inputs);

    context["show_inputs"]   = show_key_images;
    context["inputs_no"]     = static_cast<uint64_t>(inputs.size());
    context["sum_mixin_EXO"] = exohoodeg::EXO_amount_to_str(
            sum_mixin_EXO, "{:0.2f}", false);
    ...
    // useful for unit testing as it provides total EXO sum
    // of possible mixins
    ...
    if (sum_mixin_EXO > (sum_EXO + txd.fee)
        && no_of_matched_mixins == inputs.size())
    {
        //                  (outcoming    - incoming) - fee
        possible_spending = (sum_mixin_EXO - sum_EXO) - txd.fee;
    }

    context["possible_spending"] = exohoodeg::EXO_amount_to_str(
           ...
           string domain)
{
    ...
    return show_my_outputs(tx_hash_str, EXO_address_str,
...
show_checkrawtx(string raw_tx_data, string action)
{
    ...
    string data_prefix = exohoodeg::make_printable(decoded_raw_tx_data.substr(0, magiclen));
        ...
        if (r)
        {
            ...
            for (const ::tools::wallet2::tx_construction_data& tx_cd: exported_txs.txes)
            {
                ...
                mstch::map tx_cd_data {
                        ...
                        {"change_amount"      , exohoodeg::EXO_amount_to_str(tx_change.amount)},
                        {"has_payment_id"     , (payment_id  != null_hash)},
                        {"has_payment_id8"    , (payment_id8 != null_hash8)},
                        {"payment_id"         , pid_str},
                        {"payment_id8"        , pid8_str},
                ...
                for (const tx_destination_entry& a_dest: tx_cd.splitted_dsts)
                {
                    mstch::map dest_info {
                            ...
                            {"dest_amount"   , exohoodeg::EXO_amount_to_str(a_dest.amount)}
                    };
                ...
                for (size_t i = 0; i < no_of_sources; ++i)
                {
                    ...
                    mstch::map single_dest_source {
                            {"output_amount"              , exohoodeg::EXO_amount_to_str(tx_source.amount)},
                ...
                tx_cd_data.insert({"sum_outputs_amounts" ,
                                   exohoodeg::EXO_amount_to_str(sum_outputs_amounts)});
                ...
                tx_cd_data["min_mix_time"]     = exohoodeg::timestamp_to_str_gm(min_mix_timestamp);
                tx_cd_data["max_mix_time"]     = exohoodeg::timestamp_to_str_gm(max_mix_timestamp);
    ...
    else
    {
        ...
        if (strncmp(decoded_raw_tx_data.c_str(), SIGNED_TX_PREFIX, magiclen) != 0)
        {
            ...
            // used in myexohood and openexohood projects.
...
//                serialization::serialize(ba, tx_from_blob);
        ...
        for (tools::wallet2::pending_tx& ptx: ptxs)
        {
            ...
            if (ptx.construction_data.change_dts.amount > 0)
            {
                destination_addresses.push_back(
                        mstch::map {
                                ...
                                {"dest_amount"    ,
                                        exohoodeg::EXO_amount_to_str(ptx.construction_data.change_dts.amount)},
                                {"is_this_change" , true}
                        }
                );
            ...
            tx_context["outputs_EXO_sum"] = exohoodeg::EXO_amount_to_str(outputs_EXO_sum);
            ...
            for (size_t i = 0; i < outputs.size(); ++i)
            {
                ...
                if (parse_amount(output_amount, out_amount_str))
                {
                    if (output_amount == 0)
                    {
                        out_amount_str = exohoodeg::EXO_amount_to_str(real_ammounts.at(i));
                    }
                }
            }
            ...
            for (const tx_source_entry&  tx_source: ptx.construction_data.sources)
            {
                ...
                inputs_EXO_sum += tx_source.amount;
            }

            // mark that we have signed tx data for use in mstch
            tx_context["have_raw_tx"] = true;

            // provide total mount of inputs EXO
            tx_context["inputs_EXO_sum"] = exohoodeg::EXO_amount_to_str(inputs_EXO_sum);
            ...
            for (mstch::node& input_node: inputs)
            {
                ...
                amount = exohoodeg::EXO_amount_to_str(real_amounts.at(input_idx));
...
show_pushrawtx(string raw_tx_data, string action)
{
    ...
    // if failed, treat raw_tx_data as base64 encoding of signed_exohood_tx
    else
    {
        string decoded_raw_tx_data = epee::string_encoding::base64_decode(raw_tx_data);
...
show_checkrawkeyimgs(string raw_data, string viewkey_str)
{
    ...
    if (!exohoodeg::parse_str_secret_key(viewkey_str, prv_view_key))
    {
    ...
    string data_prefix = exohoodeg::make_printable(decoded_raw_data.substr(0, magiclen));
    ...
    decoded_raw_data = exohoodeg::decrypt(
    ...
    // get EXO address stored in this key image file
    const account_public_address* EXO_address =
            reinterpret_cast<const account_public_address*>(
                    decoded_raw_data.data());

    address_parse_info address_info {*EXO_address, false};


    context.insert({"address"        , REMOVE_HASH_BRAKETS(
            exohoodeg::print_address(address_info, nettype))});
    context.insert({"viewkey"        , REMOVE_HASH_BRAKETS(
            fmt::format("{:s}", prv_view_key))});
    context.insert({"has_total_EXO"  , false});
    context.insert({"total_EXO"      , string{}});
    ...
    for (size_t n = 0; n < no_key_images; ++n)
    {
        ...
        mstch::map key_img_info {
                ...
                {"address"             , exohoodeg::print_address(
...
show_checkcheckrawoutput(string raw_data, string viewkey_str)
{
    ...
    address_parse_info address_info {*EXO_address, false, false, crypto::null_hash8};
    ...
    for (const tools::wallet2::transfer_details& td: outputs)
    {
        ...
        if (td.is_rct())
        {
            ...
            public_key tx_pub_key = exohoodeg::get_tx_pub_key_from_received_outs(tx);
        ...
        mstch::map output_info {
                ...
                {"amount"              , exohoodeg::EXO_amount_to_str(EXO_amount)},
                {"tx_hash"             , REMOVE_HASH_BRAKETS(fmt::format("{:s}", td.m_txid))},
                {"timestamp"           , exohoodeg::timestamp_to_str_gm(blk_timestamp)},
    ...
    if (total_EXO > 0)
    {
        context["has_total_EXO"] = true;
        context["total_EXO"] = exohoodeg::EXO_amount_to_str(total_EXO);
    }
...
search(string search_text)
{
    ...
    // check if exohood address is given based on its length
    ...
    if (search_str_length == 98)
    {
        ...
        if (!exohoodeg::parse_str_address(search_text, address_info, nettype_addr))
        {
    ...
    // check if integrated exohood address is given based on its length
...
show_address_details(const address_parse_info& address_info, cryptonote::network_type nettype = cryptonote::network_type::MAINNET, size_t page_no = 1)
{
    ...
    string address_str      = exohoodeg::print_address(address_info, nettype);
    ...
    public_key combined_key = exohoodeg::addKeys(address_info.address.m_view_public_key, address_info.address.m_spend_public_key);
    ...
    mstch::map context {
            {"EXO_address"        , REMOVE_HASH_BRAKETS(address_str)},
            {"is_sub_addr" , false},
            {"tx_count"    , tx_outs_map.size()},
            {"total_received"    , exohoodeg::EXO_amount_to_str(total_received)},
            {"balance"    , exohoodeg::EXO_amount_to_str(total_received - total_spent)},
    };
    ...
    while (!tx_hashes.empty() && i >= start_index)
    {
      ...
      txs.push_back(txd.get_mstch_map(exohoodeg::addKeys(address_info.address.m_view_public_key, address_info.address.m_spend_public_key)));
      --i;
    }
    ...
    for (const pair<string, vector<string>>& found_txs: all_possible_tx_hashes)
    {
        ...
        if (!found_txs.second.empty())
        {
            ...
            for (const string& tx_hash: found_txs.second)
            {
                ...
                txd_map.insert({"timestamp", exohoodeg::timestamp_to_str_gm(blk_timestamp)});
...
json_transaction(string tx_hash_str)
{
    ...
    string blk_timestamp_utc = exohoodeg::timestamp_to_str_gm(tx_timestamp);
...
json_detailedtransaction(string tx_hash_str)
{
    ...
    tx_context.erase("inputs_EXO_sum_not_zero");
...
json_block(string block_no_or_hash)
{
    ...
    else if (block_no_or_hash.length() == 64)
    {
        ...
        if (!exohoodeg::parse_str_secret_key(block_no_or_hash, blk_hash))
        {
    ...
    j_data = json {
            ...
            {"timestamp_utc" , exohoodeg::timestamp_to_str_gm(blk.timestamp)},
            {"block_height"  , block_height},
            {"size"          , blk_size},
            {"txs"           , j_txs},
             ...
             bool tx_prove = false)
{
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "exohood address not provided";
        return j_response;
    }
    ...
    if (!exohoodeg::parse_str_address(address_str,  address_info, nettype))
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant parse exohood address: " + address_str;
        return j_response;
...
json_networkinfo()
{
    ...
    if (!get_exohood_network_info(j_info))
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant get exohood network info";
        return j_response;
    }
...
json_emission()
{
    ...
    else
    {
        ...
        string emission_blk_no   = std::to_string(current_values.blk_no - 1);
        string emission_coinbase = EXO_amount_to_str(current_values.coinbase, "{:0.3f}");
        string emission_fee      = EXO_amount_to_str(current_values.fee, "{:0.4f}", false);
...
json_version()
{
    ...
    j_data = json {
            ...
            {"exohood_version_full" , string {exohood_VERSION_FULL}},
...
get_tx_json(const transaction& tx, const tx_details& txd)
{
    ...
    json j_tx {
            ...
            {"mixin"       , txd.mixin_no},
            {"tx_size"     , txd.size},
            {"EXO_outputs" , txd.EXO_outputs},
            {"EXO_inputs"  , txd.EXO_inputs},
...
construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
{
    ...
    double payed_for_kB = EXO_AMOUNT(txd.fee) / tx_size;
    ...
    mstch::map context {
            ...
            {"tx_fee"                , exohoodeg::EXO_amount_to_str(txd.fee, "{:0.2f}", false)},
            {"tx_fee_micro"          , exohoodeg::EXO_amount_to_str(txd.fee*1e6, "{:0.4f}", false)},
    ...
    for (const txin_to_key &in_key: txd.input_key_imgs)
    {
        ...
        inputs.push_back(mstch::map {
                ...
                {"amount"       , exohoodeg::EXO_amount_to_str(in_key.amount)},
        ...
        for (const uint64_t& i: absolute_offsets)
        {
            ...
            if (detailed_view)
            {
                ...
                mixins.push_back(mstch::map {
                        ...
                        {"mix_timestamp",  exohoodeg::timestamp_to_str_gm(blk.timestamp)},
    ...
    if (detailed_view)
    {
        ...
        context["min_mix_time"]     = exohoodeg::timestamp_to_str_gm(min_mix_timestamp);
        context["max_mix_time"]     = exohoodeg::timestamp_to_str_gm(max_mix_timestamp);
      ...
      for (auto i = 0; i < tx.vin.size(); i++)
      {
        ...
        else
        {
          ...
          try
          {
            ...
            amount = exohoodeg::EXO_amount_to_str(in_public.amount);
          ...
          inputs_EXO_sum += in_public.amount;
    ...
    context["inputs_EXO_sum_not_zero"]  = (inputs_EXO_sum > 0);
    context["inputs_EXO_sum"]           = exohoodeg::EXO_amount_to_str(inputs_EXO_sum);
    ...
    if(tx.version >= 2) {
      ...
      for(auto i = 0; i < tx.vout.size(); ++i) {
        ...
        outputs.push_back(mstch::map {
                {"index"          , i},
                {"amount"         , exohoodeg::EXO_amount_to_str(outp.amount)},
                {"to"             , to},
                {"is_change"      , is_change},
                {"is_spent"       , is_spent},
      ...
      for(auto m : input_address_amount)
      {
        invoice_entries.push_back(mstch::map {
            {"address",         m.first},
            {"amount",          exohoodeg::EXO_amount_to_str(m.second)},
            {"positive",        false}
        });
      }
    ...
    context["outputs_EXO_sum"] = exohoodeg::EXO_amount_to_str(outputs_EXO_sum);
    context.emplace("outputs", outputs);
...
)
{
    ...
    for (auto& mixn_timestamps : mixin_timestamp_groups)
    {
        ...
        pair<string, double> mixin_times_scale = exohoodeg::timestamps_time_scale(
                mixn_timestamps,
                max_mix_timestamp,
                170,
                min_mix_timestamp);
               ...
               uint64_t bc_height = 0)
{
    ...
    exohood::basic::Validator v = core_storage->get_validator_by_height(blk_height);

    txd.v_name = v.gEXOame().empty() ? "?" : v.gEXOame();
...
bool
get_exohood_network_info(json& j_info)
{
    MempoolStatus::network_info local_copy_network_info
        = MempoolStatus::current_network_info;
...
get_footer()
{
    ...
    string footer_html = mstch::render(exohoodeg::read(TMPL_FOOTER), footer_context);
