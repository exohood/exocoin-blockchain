...
#ifndef EXOEG01_TOOLS_H
#define EXOEG01_TOOLS_H

#define PATH_SEPARARTOR '/'

#define EXO_AMOUNT(value) \
...
static
string
EXO_amount_to_str(const uint64_t& EXO_amount,
                  string _format="{:0.2f}",
                  bool zero_to_question_mark=true)
{
    ...
    {
        amount_str = fmt::format(_format, EXO_AMOUNT(EXO_amount));
    }
    else
    {
        if (EXO_amount > 0 && zero_to_question_mark == true)
        {
            amount_str = fmt::format(_format, EXO_AMOUNT(EXO_amount));
        }
    }
...
static
string
EXO_amount_to_str_formated(const uint64_t& EXO_amount,
                  string _format="{:0.2f}",
                  bool zero_to_question_mark=true)
{
    string amount_str = "?";
