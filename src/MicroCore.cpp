...
MicroCore::init(const string& _blockchain_path, network_type nt)
{
    ...
    if(m_blockchain_storage.init(db, nettype)) {
        validators = std::unique_ptr<exohood::basic::Validators>(new exohood::basic::Validators(*db, nullptr, nt == TESTNET));
...
MicroCore::get_addr_outputs(const public_key &view_key, const public_key &spend_key)
{
  ...
  try
  {
    public_key combined_key = exohoodeg::addKeys(view_key, spend_key);
