...
#ifndef CROWEXO_RPCCALLS_H
#define CROWEXO_RPCCALLS_H
...
{
// declare struct in exohood's cryptonote namespace.
// exohood should provide definition for this,
...
public:
    ...
    get_alt_blocks(vector<string>& alt_blocks_hashes)
    {
        ...
        if (r)
        {
            ...
            if (!err.empty())
            {
                cerr << "Error connecting to exohood deamon due to "
                     << err << endl;
                return false;
            }
        }
        ...
        {
            cerr << "Error connecting to exohood deamon at "
                 << deamon_url << endl;
            return false;
        }
