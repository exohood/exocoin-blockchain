...
MempoolStatus::read_mempool()
{
    ...
    for (size_t i = 0; i < mempool_tx_info.size(); ++i)
    {
        ...
        double payed_for_kB = exo_AMOUNT(_tx_info.fee) / tx_size;
        ...
        last_tx.fee_str          = exohoodeg::exo_amount_to_str(_tx_info.fee, "{:0.2f}", false);
        last_tx.fee_micro_str    = exohoodeg::exo_amount_to_str(_tx_info.fee, "{:0.2f}", false);
        last_tx.payed_for_kB_str = fmt::format("{:0.2f}", payed_for_kB);
        last_tx.payed_for_kB_micro_str = fmt::format("{:02.0f}", payed_for_kB*1e6);
        last_tx.exo_inputs_str   = exohoodeg::exo_amount_to_str(last_tx.sum_inputs , "{:0.2f}");
        last_tx.exo_outputs_str  = exohoodeg::exo_amount_to_str(last_tx.sum_outputs, "{:0.2f}");
        last_tx.timestamp_str    = exohoodeg::timestamp_to_str_gm(_tx_info.receive_time);
...
bf::path MempoolStatus::blockchain_path {"/home/ubuntu/.exohood/lmdb"};
...
exohoodeg::MicroCore*  MempoolStatus::mcore {nullptr};
