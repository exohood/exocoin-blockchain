...
using exochain::remove_bad_chars;
...
main(int ac, const char* av[])
{
    ...
    exochain::CmdLineOptions opts {ac, av};
    ...
    if (!exochain::get_blockchain_path(bc_path_opt, blockchain_path, nettype))
    {
    ...
    exochain::MicroCore mcore;
    cryptonote::Blockchain* core_storage;

    // initialize mcore and core_storage
    if (!exochain::init_blockchain(blockchain_path.string(),
    ...
    if (enable_emission_monitor == true)
    {
        ...
        exochain::CurrentBlockchainStatus::blockchain_path
                = blockchain_path;
        exochain::CurrentBlockchainStatus::nettype
                = nettype;
        exochain::CurrentBlockchainStatus::deamon_url
                = deamon_url;
        exochain::CurrentBlockchainStatus::set_blockchain_variables(
                &mcore, core_storage);
        ...
        exochain::CurrentBlockchainStatus::start_monitor_blockchain_thread();
    }


    exochain::MempoolStatus::blockchain_path
            = blockchain_path;
    exochain::MempoolStatus::nettype
            = nettype;
    exochain::MempoolStatus::deamon_url
            = deamon_url;
    exochain::MempoolStatus::set_blockchain_variables(
            &mcore, core_storage);

    exochain::MempoolStatus::network_info initial_info;
    strcpy(initial_info.block_size_limit_str, "0.0");
    strcpy(initial_info.block_size_median_str, "0.0");
    exochain::MempoolStatus::current_network_info = initial_info;
    ...
    // by tx searching threads that are launched for each user independently,
    // when they log back or create new account.
    exochain::MempoolStatus::mempool_refresh_time = mempool_refresh_time;
    exochain::MempoolStatus::start_mempool_status_thread();

    // create instance of page class which
    // contains logic for the website
    exochain::page exoblocks(&mcore,
                          core_storage,
                          deamon_url,
                          nettype,
                          enable_pusher,
    ...
    CROW_ROUTE(app, "/")
    ([&]() {
        return myexo::htmlresponse(exoblocks.index2());
    });

    CROW_ROUTE(app, "/page/<uint>")
    ([&](size_t page_no) {
        return myexo::htmlresponse(exoblocks.index2(page_no));
    });

    CROW_ROUTE(app, "/block/<uint>")
    ([&](size_t block_height) {
        return myexo::htmlresponse(exoblocks.show_block(block_height));
    });

    CROW_ROUTE(app, "/block/<uint>/page/<uint>")
    ([&](size_t block_height, size_t page_no) {
        return myexo::htmlresponse(exoblocks.show_block(block_height, page_no));
    });

    CROW_ROUTE(app, "/block/<string>")
    ([&](string block_hash) {
        return myexo::htmlresponse(
                exoblocks.show_block(remove_bad_chars(block_hash)));
    });

    CROW_ROUTE(app, "/block/<string>/page/<uint>")
    ([&](string block_hash, size_t page_no) {
        return myexo::htmlresponse(
                exoblocks.show_block(remove_bad_chars(block_hash), page_no));
    });

    CROW_ROUTE(app, "/tx/<string>")
    ([&](string tx_hash) {
        return myexo::htmlresponse(
                exoblocks.show_tx(remove_bad_chars(tx_hash)));
    });
    if (enable_autorefresh_option)
    {
        ...
        ([&](string tx_hash) {
            ...
            return myexo::htmlresponse(
                exoblocks.show_tx(remove_bad_chars(tx_hash), with_ring_signatures, refresh_page));
        });
    }
    ...
    if (enable_as_hex)
    {
        ...
        ([&](string tx_hash) {
            return crow::response(
                    exoblocks.show_tx_hex(remove_bad_chars(tx_hash)));
        });

        CROW_ROUTE(app, "/ringmembershex/<string>")
        ([&](string tx_hash) {
            return crow::response(
                    exoblocks.show_ringmembers_hex(remove_bad_chars(tx_hash)));
        });

        CROW_ROUTE(app, "/blockhex/<uint>")
        ([&](size_t block_height) {
            return crow::response(
                    exoblocks.show_block_hex(block_height, false));
        });

        CROW_ROUTE(app, "/blockhexcomplete/<uint>")
        ([&](size_t block_height) {
            return crow::response(
                    exoblocks.show_block_hex(block_height, true));
        });

//        CROW_ROUTE(app, "/ringmemberstxhex/<string>")
//        ([&](string tx_hash) {
//            return crow::response(
//              exoblocks.show_ringmemberstx_hex(remove_bad_chars(tx_hash)));
//        });

        CROW_ROUTE(app, "/ringmemberstxhex/<string>")
        ([&](string tx_hash) {
            return myexo::jsonresponse {
                exoblocks.show_ringmemberstx_jsonhex(
    ...
    if (enable_autorefresh_option)
    {
        ...
        ([&](string tx_hash, uint16_t with_ring_signature) {
            bool refresh_page {true};
            return myexo::htmlresponse(
                exoblocks.show_tx(remove_bad_chars(tx_hash), with_ring_signature, refresh_page));
        });
    }

    CROW_ROUTE(app, "/myoutputs").methods("POST"_method)
    ([&](const crow::request& req) -> myexo::htmlresponse
     {

        map<std::string, std::string> post_body
                = exochain::parse_crow_post_data(req.body);
        ...
        string response = exoblocks.show_my_outputs(
                                         tx_hash, etn_address,
                                         viewkey, raw_tx_data,
                                         domain);

        return myexo::htmlresponse(std::move(response));
    });
    ...
    ([&](const crow::request& req, string tx_hash,
        string etn_address, string viewkey)
     {
        ...
        return myexo::htmlresponse(exoblocks.show_my_outputs(
                                         remove_bad_chars(tx_hash),
        ...
        ([&](const crow::request& req) -> myexo::htmlresponse 
         {
            ...
            return myexo::htmlresponse(exoblocks.show_prove(tx_hash,
                                        etn_address,
                                        tx_prv_key,
                                        raw_tx_data,
                                        domain));
    ...
    ([&](const crow::request& req, string tx_hash,
         string etn_address, string tx_prv_key) 
     {
        ...
        return myexo::htmlresponse(exoblocks.show_prove(
                                    remove_bad_chars(tx_hash),
                                    remove_bad_chars(etn_address),
    ...
    if (enable_pusher)
    {
        ...
        ([&]() {
            return myexo::htmlresponse(exoblocks.show_rawtx());
        });
        ...
        ([&](const crow::request& req) -> myexo::htmlresponse
         {
            ...
            if (action == "check")
                return myexo::htmlresponse(
                        exoblocks.show_checkrawtx(raw_tx_data, action));
            else if (action == "push")
                return myexo::htmlresponse(
                        exoblocks.show_pushrawtx(raw_tx_data, action));
    ...
    if (enable_key_image_checker)
    {
        ...
        ([&]() {
            return myexo::htmlresponse(exoblocks.show_rawkeyimgs());
        });
        ...
        ([&](const crow::request& req) -> myexo::htmlresponse
         {
            ...
            return myexo::htmlresponse(
                    exoblocks.show_checkrawkeyimgs(raw_data, viewkey));
        });
    }
    ...
    if (enable_output_key_checker)
    {
        ...
        ([&]() {
            return myexo::htmlresponse(exoblocks.show_rawoutputkeys());
        });
        ...
        ([&](const crow::request& req) -> myexo::htmlresponse
         {
            ...
            return myexo::htmlresponse(
                    exoblocks.show_checkcheckrawoutput(raw_data, viewkey));
        });
    }
    ...
    ([&]() {
        return myexo::htmlresponse(exoblocks.mempool(true));
    });
    ...
    ([&](string address) -> myexo::htmlresponse
     {
         ...
         if (!exochain::parse_str_address(address, address_info, nettype_addr))
         {
           cerr << "Cant parse string address: " << address << endl;
           return string("Cant parse address (probably incorrect format): ")
                  + address;
         }
         return myexo::htmlresponse(exoblocks.show_address_details(address_info));
     });

    CROW_ROUTE(app, "/address/<string>/page/<uint>")
    ([&](string address, size_t page_no) -> myexo::htmlresponse
     {
        ...
        address = remove_bad_chars(address);
         ...
         return myexo::htmlresponse(exoblocks.show_address_details(address_info, nettype_addr, page_no));
     });

//    CROW_ROUTE(app, "/altblocks")
//    ([&](const crow::request& req) {
//        return exoblocks.altblocks();
//    });
    ...
    ([&](std::string css) {
        return exochain::read("./templates/css/" + css);
    });

    CROW_ROUTE(app, "/imgs/<string>")
    ([&](std::string img) {
        return exochain::read("./templates/imgs/" + img);
    });

    CROW_ROUTE(app, "/fonts/<string>")
    ([&](std::string font) {
        return exochain::read("./templates/fonts/" + font);
    });
    ...
    if (enable_js)
    {
        ...
        ([&]() {
            return exoblocks.get_js_file("jquery.min.js");
        });

        CROW_ROUTE(app, "/js/crc32.js")
        ([&]() {
            return exoblocks.get_js_file("crc32.js");
        });

        CROW_ROUTE(app, "/js/biginteger.js")
        ([&]() {
            return exoblocks.get_js_file("biginteger.js");
        });

        CROW_ROUTE(app, "/js/crypto.js")
        ([&]() {
            return exoblocks.get_js_file("crypto.js");
        });

        CROW_ROUTE(app, "/js/config.js")
        ([&]() {
            return exoblocks.get_js_file("config.js");
        });

        CROW_ROUTE(app, "/js/nacl-fast-cn.js")
        ([&]() {
            return exoblocks.get_js_file("nacl-fast-cn.js");
        });

        CROW_ROUTE(app, "/js/base58.js")
        ([&]() {
            return exoblocks.get_js_file("base58.js");
        });

        CROW_ROUTE(app, "/js/cn_util.js")
        ([&]() {
            return exoblocks.get_js_file("cn_util.js");
        });

        CROW_ROUTE(app, "/js/sha3.js")
        ([&]() {
            return exoblocks.get_js_file("sha3.js");
        });
        ...
        ([&]() {
            ...
            return exoblocks.get_js_file("all_in_one.js");
        });
    ...
    if (enable_json_api)
    {
        ...
        ([&](string tx_hash) {
            ...
            myexo::jsonresponse r{exoblocks.json_transaction(remove_bad_chars(tx_hash))};
        ...
        ([&](string tx_hash) {
            ...
            myexo::jsonresponse r{exoblocks.json_rawtransaction(remove_bad_chars(tx_hash))};
        ...
        ([&](string tx_hash) {
            ...
            myexo::jsonresponse r{exoblocks.json_detailedtransaction(remove_bad_chars(tx_hash))};
        ...
        ([&](string block_no_or_hash) {
            ...
            myexo::jsonresponse r{exoblocks.json_block(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](string block_no_or_hash) {
            ...
            myexo::jsonresponse r{exoblocks.json_rawblock(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](const crow::request &req) {
            ...
            myexo::jsonresponse r{exoblocks.json_transactions(
        ...
        ([&](const crow::request &req) {
            ...
            myexo::jsonresponse r{exoblocks.json_mempool(
                    remove_bad_chars(page), remove_bad_chars(limit))};
        ...
        ([&](string search_value) {
            ...
            myexo::jsonresponse r{exoblocks.json_search(remove_bad_chars(search_value))};
        ...
        ([&]() {
            ...
            myexo::jsonresponse r{exoblocks.json_networkinfo()};
        ...
        ([&]() {
            ...
            myexo::jsonresponse r{exoblocks.json_emission()};
        ...
        ([&](const crow::request &req) {
            ...
            myexo::jsonresponse r{exoblocks.json_outputs(
                    remove_bad_chars(tx_hash),
        ...
        ([&](const crow::request &req) {
            ...
            myexo::jsonresponse r{exoblocks.json_outputsblocks(
        ...
        ([&]() {
            ...
            myexo::jsonresponse r{exoblocks.json_version()};
    ...
    if (enable_autorefresh_option)
    {
        ...
        ([&]() {
            uint64_t page_no {0};
            bool refresh_page {true};
            return exoblocks.index2(page_no, refresh_page);
        });
    }
    ...
    if (enable_emission_monitor == true)
    {
        ...
        exochain::CurrentBlockchainStatus::m_thread.interrupt();
        exochain::CurrentBlockchainStatus::m_thread.join();
    ...
    exochain::MempoolStatus::m_thread.interrupt();
    exochain::MempoolStatus::m_thread.join();
